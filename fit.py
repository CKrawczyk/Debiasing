...
def get_vote_fraction(data, question, answer, params):
    total = sum([data[name] for name in data.dtype.names if (question in name) and (params.count_suffix in name)])
    vote_fraction = data[question + '_' + answer + params.count_suffix] / total
    vote_fraction.name = question + '_' + answer + params.count_suffix + '_fraction'
...
class FunctionFit():
    ...
    def cumfrac_fit(self, data, function_, append=True):
        q = self.question
        a = self.answer
        fv = np.sort(get_vote_fraction(data, q, a, self.params))
        fv_nonzero = fv != 0
...
class FitToBins():
    ...
    def get_kc_function(self, verbose=True):
        ...
        output_table['chisq'][np.isfinite(output_table['chisq']) is False] = 10**8
...
def debias_data(data, params, fitted_k, fitted_c, function_,
    ...
    # fv = data[fv_column]
    fv = get_vote_fraction(data, question, answer, params)
...
def debias_by_bin(full_data, vbins, zbins, question, answer):
    ...
    data_column = get_vote_fraction(full_data, question, answer, params)
