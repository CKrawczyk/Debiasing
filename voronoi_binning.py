...
def select_data_arm_number(data, N_cut=5, p_cut=0.5, p_questions=['t01_smooth_or_features', 't02_edgeon', 't04_spiral'], p_answers=['a02_features_or_disk', 'a05_no', 'a08_spiral']):
...
relative_r = [((T-T.min())/(T.max()-T.min()))
              for T in [np.log10(vbins_table['R50']), -vbins_table['Mr']]]
